...
void rebonds_bulle(Bulle *b, int ecran_x, int ecran_y)
{
    ...
    if (b->y + b->rayon > ecran_y)
    {
        ...
        // Rebond violent, style arcade
        float rebond_min = 8.0f;   // augmente si tu veux encore plus

        // Si la bulle descend lentement → rebond forcé
        if (b->vy > -rebond_min)
            b->vy = -rebond_min;
        else
            b->vy = -b->vy * 0.85f; // amortissement plus faible = rebond plus haut
    }
